...
static var displayInventory = true; //If inv is opened.
...
function DisplayInventoryWindow(windowID:int)
{
	...
	for(var i:Transform in UpdatedList) //Start a loop for whats in our list.
	{
		...
		else //No CSheet was found (not recommended)
		{
			...
			if (Input.GetButtonDown("Jump") && item.isEquipment != true && item.tag == "hppotion") //Check to see if it was a left click.
				{
					i.GetComponent(ItemEffect).UseEffect(); //Use the effect of the item.
				}
				
			if (Input.GetButtonDown("Fire3") && item.isEquipment != true && item.tag == "shieldpotion") //Check to see if it was a left click.
				{
					i.GetComponent(ItemEffect).UseEffect(); //Use the effect of the item.
				}
		}
